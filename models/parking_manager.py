    ...
    def update_allocation_status(self, img_pro, img):
        ...
        for i, pos in enumerate(self.posList):
            ...
            if full_space_id not in self.parking_data:
                ...
                if not self.parking_data[full_space_id].get('manually_set', False):
                    self.parking_data[full_space_id]['occupied'] = is_occupied

                    # If state changed, update the timestamp
                    if is_occupied != self.parking_data[full_space_id].get('last_occupied', is_occupied):
                        self.parking_data[full_space_id]['last_state_change'] = datetime.now()
                        self.parking_data[full_space_id]['last_occupied'] = is_occupied
